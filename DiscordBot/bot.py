...
async def on_ready():
    print("=========\nConnected\n=========\n")
    print(f"Current servers: {len(bot.servers)}")
    print(f"Time on start: {ctime()}")
    print(f"Current Discord.py version: {discord.__version__}")
    await bot.change_presence(game=discord.Game(name=(f"{presenceGame} | {len(bot.servers)} servers!"), url=("https://go.twitch.tv/gdspectrix"), type=typeGame))
...
async def ping(ctx):
    ...
    pingEmbed.add_field(name="First ping:", value=f"{speed1}ms", inline=True)
    pingEmbed.add_field(name="Second ping:", value=f"{speed2}ms", inline=True)
    pingEmbed.add_field(name="Third ping:", value=f"{speed3}ms", inline=True)
    pingEmbed.add_field(name="Speed comment:", value=speedComment, inline=True)
    pingEmbed.add_field(name=">> Average speed: <<", value=f">> {speedAverage}ms <<", inline=False)
...
async def clear(ctx, number):
        try:
            ...
            clearConfirmation = await bot.say(f"**Cleared `{number}` messages from this channel**")
    ...
    if not message.author.bot and bot.user in message.mentions:
        try:
            if message.content.startswith("$" or "!" or "?" or "-" or "*" or "`" or "~" or "+" or "/" or ";" or "="): # a bunch of generic checks to see if we're being messaged something that wasn't meant for us
                ...
                print(f"Chatted with a user. Server: {message.server.name}. Time: {datetime.datetime.now().time()}")

        except KeyError:
            await bot.send_message(message.channel, "```Error: 'KeyError', make sure you gave not too little input and not too much ;)```")
        except AttributeError:
            print(f"Chatted with a user in DMs. Time: {datetime.datetime.now().time()}")
